...
class UserPlayer(Player):
    ...
    def choose_action(self, board):
        with self.has_move:
            self.has_move.wait()
            try:
                return self.next_move #executed this.
            finally:
                self.next_move = None #it gets executed no matter what.
...
def run():
    ...
    adversary = RandomAdversary(DEFAULT_SEED) #DEFAULT_SEED = 42
    board = Board(BOARD_WIDTH, BOARD_HEIGHT) #10,24
